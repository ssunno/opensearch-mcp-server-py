...
class TestTools:
    ...
    async def test_aggregation_tool(self):
        ...
        self.mock_client.search.return_value = mock_aggregation_results

        # Execute
        args = self.AggregationArgs(
            index='test-index',
            aggs={'price_stats': {'stats': {'field': 'price'}}},
        )
        result = await self._aggregation_tool(args)
    ...
    async def test_aggregation_tool_no_aggregations(self):
        ...
        assert len(result) == 1
        assert result[0]['type'] == 'text'
        assert "No aggregation results found for index 'test-index'" in result[0]['text']
        assert (
    ...
    async def test_aggregation_tool_error(self):
        ...
        # Setup
        self.mock_client.search.side_effect = Exception('Test aggregation error')
        ...
        assert len(result) == 1
        assert result[0]['type'] == 'text'
        assert 'Error executing aggregation query: Test aggregation error' in result[0]['text']
    ...
    def test_input_models(self):
        ...
        with pytest.raises(ValueError):
            self.AggregationArgs(index='test')  # Should fail without aggs
        ...
        assert (
            self.AggregationArgs(
                index='test', aggs={'avg_field': {'avg': {'field': 'value'}}}
            ).index
            == 'test'
        )
